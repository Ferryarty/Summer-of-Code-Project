...
def play(agent, board, spawn_random_tile=True, n_games=1):
...
def learn_from_gameplay(agent, gp, alpha=0.1):
    ...
    for tr in gp.transition_history[:-1][::-1]:
      if len(Board(tr.s_next).empty_tiles()) >0 :
    ...
    try:
        for i_se in range(n_session):
            for i_ep in range(n_episode):
                board = None
                play(agent,board,True,n_games)
                n_games += 1
                print("{} games played by the agent".format(n_games))
            print(f"Session {i_se + 1}/{n_session} completed")
